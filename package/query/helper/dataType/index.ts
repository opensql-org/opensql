...
export default function (
  ...
  const dataType: string = str.split(' ')[0].split('(')[0].toLowerCase();
  const mapOfDataTypeInstance: JSONFunction = {
    ...
    nvarcharmax: (): string =>
      varcharMax[dbName]?.query(str, types.nVarcharMax),
    smallserial: (): string =>
      smallSerial[dbName]?.query(str, types.smallSerial),
    smalldatetime: (): string =>
      dateTime[dbName]?.query(str, types.smallDateTime),
    datetimeoffset: (): string =>
      dateTime[dbName]?.query(str, types.dateTimeOffset),
    varbinarymax: (): string =>
      varbinaryMax[dbName]?.query(str, types.varbinaryMax),
    charactervarying: (): string =>
      characterVarying[dbName]?.query(str, types.characterVarying),
  };
