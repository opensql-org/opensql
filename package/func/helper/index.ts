...
import { Cnj, COP } from '../../enum/helper';
...
function ASCII(char: string | number): string {
  const haveSpace = typeof char === 'string' && /\u0020/.test(char);
...
function AS(data: string, columnName: string): string {
  const haveASKeyword: boolean = / AS /g.test(data);
...
function POLYGON(str: string): string {
  const haveParentheses = str.indexOf('(', 0) === 0;
  return `POLYGON(${haveParentheses ? str : `(${str})`})`;
}

function JSON(data: number[] | string[] | JSONObject | JSONObject[]) {
  const stringForArr = (data: any) =>
    `[${Util.arrayToStringWithDoubleQuotationMark(data)}]`;
  ...
  if (Array.isArray(data)) {
    return stringForArr(Util.jsonArrayToString(data));
  }
...
function qCheck(
  value: QCheckValueInObject | string | number | JsonChecker,
 ...
 */
function CONTAINS(
  target: JSONObject | string,
  candidate: JSONObject | string,
  path?: string,
): string {
  const jsonHandler = (data: JSONObject | string) =>
    typeof data === 'string' ? data : Util.jsonToString(data);

  const str = `'${jsonHandler(target)}', '${jsonHandler(candidate)}'`;
