...
function havingHandler(arr) {
    ...
    arr.forEach((item, index) => {
        ...
        if (index > 0) {
            resultArr.push(conjunctionType)
        }

        resultArr.push(leftStatement);

        resultArr.push(comparisonOperator);

        resultArr.push('__$OPTION_VALUE$__');

        pushInject(rightStatement, 'VALUE');
    });

    pushQuery(resultArr.join(' '));
}
